...
class Address {
	...
	public static function getAddressesByEmailId(PDO &$getEmailPdo, $emailId) {
		...
		while(($results = $preparedStatement->fetch()) !== false) {
			try {
				$address = 	array($results["addressId"],
										$results["emailId"],
										$results["addressAttention"],
										$results["addressStreet1"],
										$results["addressCity"],
										$results["addressState"],
										$results["addressZip"],
										$results["addressStreet2"],
										$results["addressLabel"],
										$results["addressHidden"]);
